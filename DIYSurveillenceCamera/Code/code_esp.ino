...
const char *AP_ssid = "PipsCam";
//Default is no password.If you want to set password,put your password here
const char *AP_password = "pipita123";

//Station mode you should put your ssid and password
//const char *ssid = "NOS-770B"; // Put your SSID here
//const char *password = "N3UJYHNT"; // Put your PASSWORD here
const char* ssid[] = {"NOS-770B", "WiFi2SSID", "NOS-770B"};
const char* password[] = {"N3UJYHNT", "WiFi2Password", "N3UJYHNT"};
int ssidSize = 3;
...
void setup() {
  ...
  if (wifiType == 0) {
    ...
    // Try to connect to each WiFi network in turn
    ...
    // If connection is not successful, print an error message
    if (WiFi.status() != WL_CONNECTED) {
        Serial.println("Unable to connect to WiFi");
    }
    else {
        //add mdns config
        if (!MDNS.begin("pipita")) {
        ...
        // Set static IP
        IPAddress staticIP(192, 168, 23, 12);
  ...
  // Start the server
  server.on("/foto", HTTP_GET, serverCapture);
  server.on("/video", HTTP_GET, serverStream);
